...
<section class="content">	
  <div class="row">
      ...
      <div class="box box-primary" style="padding: 1%">
        ...
        <form class="form-group" id="delegation_update_form" method="POST">
        <div class="form-group">
        <label>Yetkilendirme Tipi Adı</label>
        <label><input type='text' class='form-control' name='name' value="{{$data['name']}}"/></label>
        </div>
        <div class="box-header with-border" style="margin-bottom: 1%">
          ...
          @foreach($column as $val)
            <tr>
              ...
              <td><label>
                      ...
                      <input type="checkbox" name="{{$val->id}}_list" class="minimal" {{$data[$val->name]['r']}}>
                    </label>
              </td><td><label>
                      <input type="hidden" name="{{$val->id}}_add" value="off">
                      <input type="checkbox" name="{{$val->id}}_add" class="minimal" {{$data[$val->name]['a']}}>
                    </label>
              </td><td><label>
                      <input type="hidden" name="{{$val->id}}_update" value="off">
                      <input type="checkbox" name="{{$val->id}}_update" class="minimal" {{$data[$val->name]['u']}}>
                    </label>
              </td><td><label>
                      <input type="hidden" name="{{$val->id}}_delete" value="off">
                      <input type="checkbox" name="{{$val->id}}_delete" class="minimal" {{$data[$val->name]['d']}}>
                    </label>
              </td>
            </tr>
            <?php $sira++; ?>
        ...
        <div class='form-group'>
          ...
          <input type="hidden" name="id" value={{$data['id']}}>
            <button class='btn btn-primary' id='delegations_update_Btn'><i class="fa fa-save"></i> Kaydet</button>
          </label>
        </div>
        </form>
      </div>
